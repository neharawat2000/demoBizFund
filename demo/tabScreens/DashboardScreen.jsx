...
import { FaCheck } from "react-icons/fa"; // Importing FaCheck icon from react-icons/fa
...
const Dashboard = () => {
  ...
  const handleSelectTask = (taskId) => {
    const updatedTasks = tasks.map(task => ({
      ...task,
      isDone: task.id === taskId ? !task.isDone : false
    }));
          ...
          <div>
            ...
            <div className="bg-white mt-13 rounded-2xl space-x-6">
              ...
              <div className="flex justify-between">
                ...
                <div>
                  {tasks.map((task) => (
                    <div key={task.id} className={`flex justify-between items-center py-2 px-2 rounded-2xl ${task.isDone ? 'bg-custom-green px-4' : 'bg-background-color'} w-96 mr-8 mb-2`}>
                      <div className="flex">
                      ...
                      <button onClick={() => handleSelectTask(task.id)} className="bg-white rounded-full h-7 w-7 text-white flex items-center justify-center">
